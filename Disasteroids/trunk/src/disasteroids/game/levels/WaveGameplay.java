	...
	public void act()
	{
		...
		if ( wavePoints >= 50 && Util.getGameplayRandomGenerator().nextDouble() * spawnRate <= 0.3 )
		{
			...
			if ( Util.getGameplayRandomGenerator().nextDouble() * 8 * spawnRate <= 0.3 )
				...
				Game.getInstance().getObjectManager().addObject( new Asteroid( x, y, Util.getGameplayRandomGenerator().nextInt( 6 ) - 3, Util.getGameplayRandomGenerator().nextInt( 6 ) - 3, Util.getGameplayRandomGenerator().nextInt( 70 ) + 30, 15 ),
						false );
		}
