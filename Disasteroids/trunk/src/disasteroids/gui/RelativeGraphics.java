...
public abstract class RelativeGraphics
{
    ...
    {
        try {
            return (int) ((Game.getInstance().players.getFirst().getX() + Game.getInstance().GAME_WIDTH / 2)) % Game.getInstance().GAME_WIDTH;
    ...
    {
        try {
            return (int) (Game.getInstance().players.getFirst().getY() + Game.getInstance().GAME_HEIGHT / 2) % Game.getInstance().GAME_HEIGHT;
        } catch (Exception exception) {
