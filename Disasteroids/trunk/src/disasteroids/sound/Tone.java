...
public class Tone
{
	 ...
	 * with the values to be passed on to the speaker representing this <code>Tone</code> as a sine wave. The array will have
	 * the first 8*duration values filled
	...
	public static byte[] toByteArray( Tone[] toneList )
	{
		...
		int index = 0;
		for ( byte[] allVal : allVals )
		{
			for ( int i = 0; allVal != null && i < allVal.length; i++ )
				toBeReturned[index++] = allVal[i];
		}
		return toBeReturned;
	}
}